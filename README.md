...
To inspect pre-2013 candidate readiness without changing data, use `india-equity-data candidate-readiness --release-id <release>`; this reports release coverage, verified range, quality tier, and candidate promotion summary. For the 2006 candidate specifically, use `india-equity-data candidate-readiness --release-id <release> --candidate-start 2006-01-01`; this reports the same release context plus candidate gate-pass readiness, advisory feature warm-up state, the date's research-quality status, and strict research-ready status separately.
