...
For the bounded research release, `RECONSTRUCTED_TRADING_IDENTITY` is allowed when all dated master rows for a required security resolve to one listing episode, one exchange series, and no conflicting ISIN evidence. Multiple rows can therefore represent valid symbol or company-name history within one episode. This is a trading-continuity statement, not a claim of complete legal issuer genealogy.
