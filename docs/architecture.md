...
`india-equity-data candidate-readiness --release-id ...` is a read-only release inspection command. Without `--candidate-start`, it prints release coverage, verified range, quality tier, and the candidate promotion summary. With `--candidate-start 2006-01-01`, it prints the same release context plus the row-level candidate decision, gate-pass readiness, advisory feature warm-up state, refined monthly/session boundary evidence, research-quality status, and strict research-ready status for that configured start.
