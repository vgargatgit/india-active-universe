...
The release API joins PIT liquidity features lazily when reading `active_universe_daily.parquet`. Consumers can call `eligible_on(as_of_date, ...)` or `ranked_liquid_on(as_of_date, n, metric=...)`; ranking is not index membership.
