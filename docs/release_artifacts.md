...
Release `india_equity_data_v1.4.0` publishes the full contract plus `trading_status_intervals.parquet`, `delisting_notice_inventory.parquet`, `suspension_notice_evidence.parquet`, `suspension_notice_events.parquet`, `source_manifest.json`, and `suspension_source_manifest.json`. Unknown intervals are evidence gaps, never silently labeled suspension or delisting. Suspension and recommencement rows remain evidence records when later observed trading or incomplete event pairing prevents safe canonical status reconstruction.
