...
The completion audit validates `candidate_recommended_research_interval` fail-closed. Its `start` must equal `refined_earliest_candidate_gate_pass_boundary`, its status must match refined-boundary availability, its profile fields must match the published profile, and its `promotion_status` must remain `NOT_PROMOTED_UNLESS_PRESENT_IN_RESEARCH_QUALITY_INTERVALS`.
