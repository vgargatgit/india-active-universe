...
Phase 3 early-history extension evidence is candidate-scoped. `candidate_promotion_decisions` and `earliest_candidate_gate_pass_start` describe whether 2011, 2009, 2007, or 2006 candidate starts pass configured gates. `refined_earliest_candidate_gate_pass_boundary` and `candidate_recommended_research_interval` refine the boundary monthly/session-wise after the coarse scan. They do not by themselves promote an interval to `RESEARCH_HIGH_CONFIDENCE`; only `research_quality_intervals` defines the trusted research range for downstream use.
