...
def main() -> None:
    ...
    manifest = {"release_id": args.release_id, "project_id": "india-active-universe", "git_commit": current_commit(root), "coverage": {"observed_start": first, "observed_end": last, "security_count": len(securities), "observation_count": price_rows}, "source_coverage": {"source_verified_start": first, "source_verified_end": last, "verification_basis": "official NSE market-data files; no independent exchange calendar claim"}, "research_coverage": {"research_verified_start": "2013-01-01", "research_verified_end": last, "universe_profile": "NSE_BROAD_LIQUID_PIT_V1", "profile_version": "LIQUID_V1", "priority_scope": "LIQUID_V1_OR_HISTORICAL_TOP750"}, "component_quality": {"raw_source": "SOURCE_HIGH_CONFIDENCE", "raw_ohlcv": "RESEARCH_HIGH_CONFIDENCE", "research_universe_2013_onward": "RESEARCH_HIGH_CONFIDENCE", "terminal_events": "PARTIAL", "total_return": "PARTIAL"}, "definition": "ACTIVE_V1", "quality_tier": "DATASET_EXPLORATORY", "verified_start_date": first, "verified_end_date": last, "source_manifest_sha256": file_hash(source_manifest) if source_manifest.exists() else None, "config_sha256": file_hash(config_path) if config_path.exists() else None, "manual_override_sha256": file_hash(manual_override_path) if manual_override_path.exists() else None, "parser_versions": {"nse_bhavcopy": "nse-bhavcopy-v2", "canonicalization": "identity-v1"}, "artifacts": artifacts, "quality": {"identity_quality": dict(quality_counts), "quality_findings": 0}, "source": "NSE_OFFICIAL_BHAVCOPY"}
