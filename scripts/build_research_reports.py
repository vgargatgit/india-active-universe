...
""")
    ...
    event_text.extend(["", "`NO_PRE_EVENT_OBSERVATION` and `NO_POST_EVENT_OBSERVATION` identify listing-start or terminal-history edges. They do not create a false return and do not change the official factor or raw price. A two-sided boundary is required before a holder-value ratio is calculated.", "", "Rows remain traceable to `corporate_actions.parquet`; this report does not alter raw nominal prices."])
