...
def main() -> None:
    ...
    if args.dry_run:
        print(json.dumps({"source": str(source), "target": str(target), "artifacts": len(REQUIRED), "mode": "CACHED_PROMOTION"}, sort_keys=True))
        return
    ...
    try:
        ...
        output_manifest["build_mode"] = "CACHED_PROMOTION"
        ...
        raise
    print(json.dumps({"release_id": args.release_id, "parent_release_id": manifest.get("release_id"), "artifacts": len(output_manifest["artifacts"]), "mode": "CACHED_PROMOTION"}, sort_keys=True))
